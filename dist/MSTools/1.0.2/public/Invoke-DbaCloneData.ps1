...
.DESCRIPTION
    Invoke-DbaCloneData discovers user tables on the source (optionally filtered by schema/table),
...
#>
    ...
    try {
        Write-Log -Message "Copy all user tables from $SourceDatabase [$SourceInstance] to $DestDatabase [$DestInstance]" -Level Info 
        ...
        Write-Log -Message "Tables selected for copy: $total" -Level Info 
        ...
        foreach ($t in $tables) {
            $i++
            $fq = "[$($t.Schema)].[$($t.Name)]"
            Write-Log -Message "[$i/$total] Copying $fq ..." -Level Info 
            ...
            if ($PSCmdlet.ShouldProcess("$DestInstance/$DestDatabase", "Copy $fq")) {
                try {
                    ...
                    Write-Log -Message "FAILED to copy $fq : $msg" -Level Error 
        ...
        Write-Log -Message ("Duration to copy all user tables: {0:c}" -f $dur) -Level Warning 
        Write-Log -Message ("Summary: OK={0}  FAIL={1}" -f $ok.Count,$failed.Count) -Level Warning 

        if ($failed.Count -gt 0) {
            $failed | ForEach-Object { Write-Log -Message ("  - " + $_.Table + " :: " + $_.Error) -Level Warning }
        }
        ...
        Write-Log -Message ("ERROR (Invoke-DbaCloneData): " + $_.Exception.Message) -Level Error
        throw
    }
}