...
#>
    ...
    try {
        Write-Log -Message "Exporting Indexes + PK + FK (no table creation)" -Level Info 
        ...
        Write-Log -Message "Index/PK/FK script: $IdxPkFkScriptPath (Indexes=$($indexObjects.Count), FKs=$($fkObjects.Count))" -Level Info 

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy Indexes + PK + FK")) {
            Write-Log -Message "Deploying Indexes + PK + FK into $DestDatabase on $DestInstance" -Level Warning 
            [void](Invoke-DbaExecuteBatchWithRetry -SqlInstance $DestInstance -Database $DestDatabase -FilePath $IdxPkFkScriptPath -MaxPasses $MaxPasses)
        }

        Write-Log -Message "Indexes + PK + FK deployment completed." -Level Warning 
        ...
        Write-Log -Message ("ERROR (Invoke-DbaClonePkFk): " + $_.Exception.Message) -Level Error 
        throw
    }
}
