function Ensure-TargetDatabase {
<#
.SYNOPSIS
...
.EXAMPLE
Ensure-TargetDatabase -DestInstance "ISI-M0BDRS31" -NewDatabase "MyAppDB" -DropIfExists -Confirm:$false

Drops and recreates [ISI-M0BDRS31].[MyAppDB] without prompting (because -Confirm:$false).

.EXAMPLE
Ensure-TargetDatabase -DestInstance "ISI-M0BDRS31" -NewDatabase "MyAppDB" -PassThru
...
#>
    ...
    Write-Log -Message "Ensure-TargetDatabase started for [$DestInstance].[$NewDatabase]." -Level Info  
    ...
    try {
        ...
        if ($existing) {
            if ($DropIfExists) {
                if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "Drop existing database")) {
                    Write-Log -Message "Dropping existing database '$NewDatabase' on '$DestInstance'." -Level Important 
        ...
        Write-Log -Message "Target database '$NewDatabase' is ready on '$DestInstance'." -Level Alert  
