...
.DESCRIPTION
    ...
    Key behaviors:
      ...
      - Creates the parent directory for -PkScriptPath when missing, overwrites existing file.
...
.PARAMETER LogFileName
    Optional path to a log file. Verbose streams and Write-Log messages are appended here.
...
.EXAMPLES
    ...
    Invoke-DbaClonePrimaryKey `
        ...
        -PkScriptPath "C:\out\AppDB-PKs.sql"
    ...
    Invoke-DbaClonePrimaryKey `
        ...
        -PkScriptPath "D:\scripts\Sales-PKs.sql" `
        -PassThru
    ...
    Invoke-DbaClonePrimaryKey `
        ...
        -PkScriptPath "E:\out\HR-PKs.sql" `
        -LogFileName "E:\logs\HR-PKs.log"
    ...
    Invoke-DbaClonePrimaryKey `
        ...
        -PkScriptPath "C:\out\Ops-PKs.sql" `
        -MaxPasses 20
...
.NOTES
    ...
    Behavior & detection:
      ...
      - Existing -PkScriptPath is removed before writing a fresh file.
...
#>
    ...
    param(
        ...
        [Parameter(Mandatory)][string]$PkScriptPath,   # output .sql for PKs only
        [int]$MaxPasses = 10,
    ...
    $dir = Split-Path -Path $PkScriptPath -Parent
    if ($dir -and -not (Test-Path -LiteralPath $dir)) {
        New-Item -ItemType Directory -Force -Path $dir | Out-Null
    }

    #try {
        Write-Log -Message "Exporting ONLY Primary Keys (no table creation)" -Level Info -LogFileName $LogFileName -Verbose
        ...
        Write-Log -Message "Found $($pkIndexes.Count) primary keys to script." -Level Info -LogFileName $LogFileName -Verbose
        ...
        if (Test-Path $PkScriptPath) { Remove-Item $PkScriptPath -Force -ErrorAction SilentlyContinue }
        if ($pkIndexes.Count -gt 0) {
            $pkIndexes | Export-DbaScript -FilePath $PkScriptPath -Append:$false `
                -ScriptingOptionsObject $opt -EnableException -Confirm:$false -Verbose *>> $LogFileName
        }

        Write-Log -Message "Primary Key script written: $PkScriptPath" -Level Info -LogFileName $LogFileName -Verbose

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy Primary Keys")) {
            Write-Log -Message "Deploying Primary Keys into $DestDatabase on $DestInstance" -Level Important -LogFileName $LogFileName
            [void](Invoke-DbaExecuteBatchWithRetry -SqlInstance $DestInstance -Database $DestDatabase -FilePath $PkScriptPath -MaxPasses $MaxPasses -LogFile $LogFileName)
        }

        Write-Log -Message "Primary Key deployment completed." -Level Alert -LogFileName $LogFileName -Verbose
        ...
        if ($PassThru) {
            [pscustomobject]@{
                ...
                ScriptPath      = $PkScriptPath
                MaxPasses       = $MaxPasses
            }
        }
    #}
    #catch {
    #    Write-Log -Message ("ERROR (Invoke-DbaClonePrimaryKey): " + $_.Exception.Message) -Level Error -LogFileName $LogFileName
    #    throw
    #}
}
