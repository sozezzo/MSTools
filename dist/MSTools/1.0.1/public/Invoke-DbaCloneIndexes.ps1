...
.DESCRIPTION
    ...
    Key behaviors:
      ...
      - The script folder is created if it doesn’t exist; any existing script at -IdxScriptPath is overwritten.
...
.EXAMPLES
    ...
    Invoke-DbaCloneIndexes `
        ...
        -IdxScriptPath "C:\temp\AppDB-indexes.sql"
    ...
    Invoke-DbaCloneIndexes `
        ...
        -IdxScriptPath "D:\out\Sales-indexes.sql" `
        -PassThru

    # 3) Exclude large audit tables and log actions
    Invoke-DbaCloneIndexes `
        ...
        -IdxScriptPath "E:\scripts\HR-indexes.sql" `
        -LogFileName "E:\scripts\HR-indexes.log"

    # 4) Dry run (script only, do not deploy)
    Invoke-DbaCloneIndexes `
        -SourceInstance "SQL01" -SourceDatabase "AppDB" `
        -DestInstance   "SQL02" -DestDatabase   "AppDB_QA" `
        -IdxScriptPath "C:\out\AppDB-indexes.sql" `
        -WhatIf
    ...
    Invoke-DbaCloneIndexes `
        ...
        -IdxScriptPath "C:\out\Ops-indexes.sql" `
        -MaxPasses 20
...
.NOTES
    ...
    Error handling:
      ...
      - Existing -IdxScriptPath is removed before writing a fresh file.
...
#>
    ...
    param(
        ...
        [Parameter(Mandatory)][string]$IdxScriptPath,   # output .sql for indexes ONLY
        [int]$MaxPasses = 10,
    ...
    $dir = Split-Path -Path $IdxScriptPath -Parent
    if ($dir -and -not (Test-Path -LiteralPath $dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }

    try {
        Write-Log -Message "Exporting ONLY non-constraint indexes (exclude PK and UNIQUE constraints)" -Level Info -LogFileName $LogFileName -Verbose
        ...
        Write-Log -Message "Found $($indexObjects.Count) indexes to script." -Level Info -LogFileName $LogFileName -Verbose
        ...
        if (Test-Path $IdxScriptPath) { Remove-Item $IdxScriptPath -Force -ErrorAction SilentlyContinue }
        if ($indexObjects.Count -gt 0) {
            $indexObjects | Export-DbaScript -FilePath $IdxScriptPath -Append:$false `
                -ScriptingOptionsObject $opt -EnableException -Confirm:$false -Verbose *>> $LogFileName
        }

        Write-Log -Message "Indexes-only script written: $IdxScriptPath" -Level Info -LogFileName $LogFileName -Verbose

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy indexes only")) {
            Write-Log -Message "Deploying indexes into $DestDatabase on $DestInstance" -Level Important -LogFileName $LogFileName
            [void](Invoke-DbaExecuteBatchWithRetry -SqlInstance $DestInstance -Database $DestDatabase -FilePath $IdxScriptPath -MaxPasses $MaxPasses -LogFile $LogFileName)
        }

        Write-Log -Message "Indexes-only deployment completed." -Level Alert -LogFileName $LogFileName -Verbose
        ...
        if ($PassThru) {
            [pscustomobject]@{
                ...
                ScriptPath      = $IdxScriptPath
                MaxPasses       = $MaxPasses
            }
        }
    }
    catch {
        Write-Log -Message ("ERROR (Invoke-DbaCloneIndexes): " + $_.Exception.Message) -Level Error -LogFileName $LogFileName
        throw
    }
}
