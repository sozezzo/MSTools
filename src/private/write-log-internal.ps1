...
.DESCRIPTION
    Write-LogInternal is a thin shim around your public Write-MSLog function.
    It short-circuits when internal debug is disabled to keep runtime overhead minimal.
    If Write-MSLog is temporarily unavailable (e.g., during module bootstrap),
...
.PARAMETER Trace
    When present, passes -Trace:$true through to Write-MSLog so you can opt into
...
.NOTES
    ...
    Fallback behavior:
      - If Write-MSLog is not available yet, prints via Write-Host with a clear
...
#>
    ...
    $writeLogCmd = Get-Command -Name Write-MSLog -ErrorAction SilentlyContinue
    if (-not $writeLogCmd) {
    ...
    Write-MSLog -Message ("[{0}] {1}" -f $Category, $Message) -Level Debug -Trace:$Trace
}
