function Invoke-MSCloneIndexes {
<#
.SYNOPSIS
    Scripts and deploys ONLY non-constraint indexes from a source database to a destination database.

.DESCRIPTION
    Invoke-MSCloneIndexes enumerates tables on the source database (optionally filtered),
...
.PARAMETER MaxPasses
    Maximum number of retry passes for deployment (used by Invoke-MSExecuteBatchWithRetry).
    Default: 10.
...
.PARAMETER LogFileName
    Optional log file path. Verbose streams and messages from Write-MSLog will be appended here.
...
.NOTES
    ...
    Error handling:
      - Exceptions are logged via Write-MSLog and rethrown.
...
#>
    ...
    try {
        Write-MSLog -Message "Exporting ONLY non-constraint indexes (exclude PK and UNIQUE constraints)" -Level Info 
        ...
        Write-MSLog -Message "Found $($indexObjects.Count) indexes to script." -Level Info 
        ...
        Write-MSLog -Message "Indexes-only script written: $ScriptPath" -Level Info  

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy indexes only")) {
            Write-MSLog -Message "Deploying indexes into $DestDatabase on $DestInstance" -Level Warning
        ...
        Write-MSLog -Message "Indexes-only deployment completed." -Level Warning 
        ...
        Write-MSLog -Message ("ERROR (Invoke-MSCloneIndexes): " + $_.Exception.Message) -Level Error
        throw
    }
}
