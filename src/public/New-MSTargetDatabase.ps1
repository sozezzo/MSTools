...
Writes progress to an optional log file via your Write-MSLog function.
...
Path to a log file used by your Write-MSLog function (optional).
...
.EXAMPLE
New-MSTargetDatabase -DestInstance "ISI-M0BDRS31" -NewDatabase "MyAppDB" -DropIfExists -Confirm:$false

Drops and recreates [ISI-M0BDRS31].[MyAppDB] without prompting (because -Confirm:$false).

.EXAMPLE
New-MSTargetDatabase -DestInstance "ISI-M0BDRS31" -NewDatabase "MyAppDB" -PassThru
...
#>
    ...
    Write-MSLog -Message "New-MSTargetDatabase started for [$DestInstance].[$NewDatabase]." -Level Info  
    ...
    try {
        ...
        if ($existing) {
            if ($DropIfExists) {
                if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "Drop existing database")) {
                    Write-MSLog -Message "Dropping existing database '$NewDatabase' on '$DestInstance'." -Level Warning 
        ...
        if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "Create database (instance defaults)")) {
            Write-MSLog -Message "Creating database '$NewDatabase' using instance defaults." -Level Info  
            New-DbaDatabase -SqlInstance $DestInstance -Name $NewDatabase -ErrorAction Stop -Confirm:$false | Out-Null
        }

        Write-MSLog -Message "Target database '$NewDatabase' is ready on '$DestInstance'." -Level Warning  
        ...
        Write-MSLog -Message ("ERROR: " + $_.Exception.Message) -Level Error 
        throw
    }
}
