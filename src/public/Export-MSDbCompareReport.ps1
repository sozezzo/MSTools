<#
    ...
    Examples
    --------
    Export-MSDbCompareReport -SourceInstance SQL01 -SourceDatabase DB1 `
...
function Export-MSDbCompareReport {
<#
.SYNOPSIS
  Generates an HTML report comparing structure and metadata of two SQL Server databases.

.DESCRIPTION
  Connects to two SQL Server instances via dbatools and compares the structure of the
  specified databases. Produces a self-contained HTML report highlighting differences in
  tables, columns, views, stored procedures, functions, and optionally triggers, keys,
  and indexes.

  Optional comparisons (opt-in via switches or -CompareAll):
    - Table DML triggers   (-CompareTableTriggers)
    - Database DDL triggers (-CompareDbTriggers)
    - Primary keys & foreign keys (-CompareKeys)
    - Index definitions (-CompareIndexes)
...
.PARAMETER OutputPath
  Full path to the output HTML file, or a directory path (a timestamped filename is
  generated automatically). Defaults to DatabaseComparisonReport.html in the current directory.

.PARAMETER CompareTableTriggers
  Include table-level DML triggers in the comparison.

.PARAMETER CompareDbTriggers
  Include database-level DDL triggers in the comparison.

.PARAMETER CompareKeys
  Include primary keys and foreign keys in the comparison.

.PARAMETER CompareIndexes
  Include index definitions (excluding PK/unique-constraint indexes) in the comparison.

.PARAMETER CompareAll
  Enable all optional comparisons (equivalent to specifying all Compare* switches).
...
function New-ObjectComparisonResult {
    ...
    foreach ($name in @($src.Keys + $dst.Keys) | Sort-Object -Unique) {
        ...
        if ($defA -ceq $defB) {
            ...
            try { $r = Get-FirstStringDifference -String1 $defA -String2 $defB; Write-MSLog -Message "Object $ObjectType : $name `r`n$($r.ToString)" } catch {}
