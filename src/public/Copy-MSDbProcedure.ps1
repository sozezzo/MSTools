function Copy-MSDbProcedure {
    ...
    .EXAMPLE
        Copy-MSDbProcedure -Source "PROD-SQL01" -Destination "TEST-SQL01"
    ...
    .EXAMPLE
        Copy-MSDbProcedure -Source "PROD-SQL01" -Destination "TEST-SQL01" -Database "MyDB" -Procedure "usp_*" -Force

        Copies/overwrites all procedures whose name matches 'usp_*' from MyDB on PROD-SQL01
        to MyDB on TEST-SQL01.

    .EXAMPLE
        Copy-MSDbProcedure -Source "PROD-SQL01" -Destination "TEST-SQL01" -Database "MyDB" -Procedure "usp_*" -Force -CompareFirst
    ...
    Write-MSLog -Level Info  -Message "Copy-MSDbProcedure started. Source=[$Source], Destination=[$Destination], Database=[$Database], Force=[$Force], CompareFirst=[$CompareFirst], Procedures=[${($Procedure -join ', ')}]"
    Write-MSLog -Level Debug -Message "Copy-MSDbProcedure parameters: $(($PSBoundParameters.GetEnumerator() | ForEach-Object { "$($_.Key)=$($_.Value)" }) -join '; ')"
...
"@
    ...
    Write-MSLog -Level Debug -Message "Querying source procedures from [$Source]. Database=[$Database]."
    $srcProcs = Invoke-DbaQuery -SqlInstance $Source -Database $Database -Query $srcQuery

    if (-not $srcProcs) {
        Write-MSLog -Level Warning -Message "No user procedures found on source [$Source] in database [$Database]. Nothing to copy."
        return
    }

    # Optional filter by -Procedure (PowerShell wildcards on ProcName)
    if ($Procedure) {
        Write-MSLog -Level Debug -Message "Filtering source procedures by patterns: $($Procedure -join ', ')"
    ...
    if (-not $srcProcs) {
        Write-MSLog -Level Warning -Message "After filtering by -Procedure, no source procedures remain. Nothing to copy."
        return
    }

    Write-MSLog -Level Info -Message ("Found {0} source procedures to consider for copy." -f $srcProcs.Count)
...
"@
    ...
    Write-MSLog -Level Debug -Message "Querying destination procedures from [$Destination]. Database=[$Database]."
    ...
    foreach ($p in $srcProcs) {
        ...
        Write-MSLog -Level Debug -Message "Processing procedure [$fullName]. ExistsOnDestination=[$existsOnDest]."

        if ($existsOnDest -and -not $Force) {
            # Only copy "new" procedures when Force is not specified
            Write-MSLog -Level Info -Message "Skipping existing procedure [$fullName] on destination [$Destination] (Force not specified)."
            $skippedExisting++
        ...
        if ($existsOnDest -and $Force -and $CompareFirst) {
            ...
            if ($srcNorm -eq $destNorm) {
                Write-MSLog -Level Info -Message "Skipping procedure [$fullName] on destination [$Destination]; definitions are identical (CompareFirst)."
                $skippedSame++
                $results += [pscustomobject]@{
                    Source       = $Source
                    Destination  = $Destination
                ...
                Write-MSLog -Level Debug -Message "Definitions differ for [$fullName]; will replace (Force + CompareFirst)."
            }
        }
...
"@
        ...
        if ($PSCmdlet.ShouldProcess("$Destination / $Database", "Create or replace procedure [$fullName] ($action)")) {
            Write-MSLog -Level Info -Message "Executing T-SQL batch for procedure [$fullName] on destination [$Destination] (Action=$action)."
            ...
            Write-MSLog -Level Info -Message "WhatIf/Confirm prevented execution for procedure [$fullName]."
            $results += [pscustomobject]@{
                Source       = $Source
                Destination  = $Destination
                Database     = $Database
    ...
    Write-MSLog -Level Info -Message ("Copy-MSDbProcedure finished. Created={0}, Replaced={1}, SkippedExisting={2}, SkippedSameDefinition={3}" -f $created, $replaced, $skippedExisting, $skippedSame)
