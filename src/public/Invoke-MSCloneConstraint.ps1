function Invoke-MSCloneConstraint {
<#
.SYNOPSIS
...
.DESCRIPTION
    Invoke-MSCloneConstraint enumerates tables in the source database (optionally filtered),
...
.PARAMETER MaxPasses
    Maximum number of retry passes when deploying the script (used by Invoke-MSExecuteBatchWithRetry).
    Default: 10.
...
.NOTES
    Requirements:
      - dbatools module:
            ...
            Export-DbaScript
      - A Write-MSLog function supporting: -Message, -Level, -LogFileName.
      - Helper: Invoke-MSExecuteBatchWithRetry
    ...
    Error handling:
      - Errors are logged via Write-MSLog and rethrown for upstream handling.
...
#>
    ...
    try {
        Write-MSLog -Message "Exporting ONLY constraints (FK, UNIQUE, CHECK, DEFAULT) � excluding PK and non-constraint indexes" -Level Info  
		...
		Write-MSLog -Message ("Constraints found -> FK:{0}  UNIQUE:{1}  CHECK:{2}  DEFAULT:{3}" -f $fkObjects.Count, $uniqueObjects.Count, $checkObjects.Count, $defaultObjects.Count) -Level Info 
        ...
        Write-MSLog -Message "Constraints script written: $ScriptPath" -Level Info  -Verbose

        # Deploy with retry
		Write-MSLog -Message "Deploying constraints into $DestDatabase on $DestInstance" -Level Warning 
        ...
        Write-MSLog -Message "Constraints deployment completed." -Level Warning  
        if ($PassThru) {
            [pscustomobject]@{
        ...
        Write-MSLog -Message "ERROR (Invoke-MSCloneConstraint): $message" -Level Error 
