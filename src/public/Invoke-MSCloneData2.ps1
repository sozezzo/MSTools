...
"@ -As SingleValue
    ...
    Write-MSLog -Message ("Copy user tables from {0} [{1}] to {2} [{3}] (Mode={4})" -f $SourceDatabase,$SourceInstance,$DestDatabase,$DestInstance,$mode) -Level Info

    $tables = Get-SourceTables
    $total  = ($tables | Measure-Object).Count
    Write-MSLog -Message ("Tables selected for copy: {0}" -f $total) -Level Info
    ...
    foreach ($t in $tables) {
        ...
        $tbl = $t.Name
        $fq  = "[$sch].[$tbl]"
        Write-MSLog -Message ("[{0}/{1}] Copying {2} ..." -f $i,$total,$fq) -Level Info
...
"@
                            ...
                            Write-MSLog -Message ("  -> {0} [{1}..{2}] copied" -f $fq,$startK,$endK) -Level Info
                            $startK = $endK + 1
                        }
                    } else {
                        Write-MSLog -Message ("No batch bounds detected for {0}; falling back to single-shot." -f $fq) -Level Warning
...
"@
            ...
            Write-MSLog -Message ("FAILED to copy {0} : {1}" -f $fq,$msg) -Level Error
    ...
    $dur = (Get-Date) - $start
    Write-MSLog -Message ("Duration to copy tables: {0:c}" -f $dur) -Level Warning
    Write-MSLog -Message ("Summary: OK={0}  FAIL={1}" -f $ok.Count,$failed.Count) -Level Warning

    if ($failed.Count -gt 0) {
        $failed | ForEach-Object { Write-MSLog -Message ("  - {0} :: {1}" -f $_.Table,$_.Error) -Level Warning }
    }
