function Compare-MSDbObject {
<#
.SYNOPSIS
    Compares object presence and key metadata between two SQL Server databases and returns a list of differences.

.DESCRIPTION
    Compare-MSDbObject inspects a broad set of database objects on a source and a destination:
...
.EXAMPLES
    ...
    $diff = Compare-MSDbObject -SourceInstance "SQL01" -SourceDatabase "Sales" `
...
.NOTES
    ...
    Return semantics:
      - Always returns an array (possibly empty). To check for any differences:
            if ((Compare-MSDbObject ...).Count) { 'Differences found' } else { 'No differences' }
