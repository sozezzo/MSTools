function Invoke-MSCloneDatabaseStructure {
<#
.SYNOPSIS
...
.NOTES
    ...
    Error Handling:
      - Failures are logged via Write-MSLog and rethrown for upstream handling.
...
#>
    ...
    try {
        ...
        Write-MSLog -Message "Script base objects (no collations in the script)" -Level Info  
        ...
        Write-MSLog -Message "Exporting base schema to '$ScriptPath'." -Level Info  
        @(
        ...
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy base schema from $ScriptPath")) {
            Write-MSLog -Message "Deploy base schema into [$DestDatabase] at [$DestInstance]." -Level Warning  
            Invoke-DbaQuery -SqlInstance $DestInstance -Database $DestDatabase -File $ScriptPath -EnableException -Verbose *>> $LogFileName
        }

        Write-MSLog -Message "Base schema deployment completed for [$DestInstance].[$DestDatabase]." -Level Warning 
        ...
        Write-MSLog -Message ("ERROR (Invoke-DbaCloneDatabase): " + $_.Exception.Message) -Level Error 
        throw
    }
}
