...
.EXAMPLE
    Invoke-MSCloneViews -SourceInstance SQL01 -SourceDatabase AppDB `
...
.EXAMPLE
    ...
    Invoke-MSCloneViews -SourceInstance SQL01 -SourceDatabase Sales `
...
#>
    ...
    process {
        try {
            Write-MSLog -Message "Collecting views from [$SourceInstance].[$SourceDatabase]." -Level Info
            ...
            if (-not $targetViews -or $targetViews.Count -eq 0) {
                Write-MSLog -Message "No views matched the provided filter." -Level Warning
                return
            }

            Write-MSLog -Message ("Views selected: " + ($targetViews | ForEach-Object { "$($_.Schema).$($_.Name)" } | Sort-Object | Out-String).Trim()) -Level Info
            ...
            if ($UseDropIfExists) {
                Write-MSLog -Message "Scripting with DROP VIEW IF EXISTS prelude per object." -Level Info
            ...
            if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy views")) {
                Write-MSLog -Message "Deploying views into [$DestInstance].[$DestDatabase]." -Level Warning
                ...
                Write-MSLog -Message "Views deployment completed." -Level Warning
            } else {
                Write-MSLog -Message "Scripting completed (no deployment due to -WhatIf / -Confirm choice)." -Level Info
            }
        }
        catch {
            Write-MSLog -Message ("ERROR (Invoke-MSCloneViews): " + $_.Exception.Message) -Level Error
            throw
        }
    }
}
