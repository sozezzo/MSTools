...
.EXAMPLE
    Export-MSAgentAlert -SqlInstance "prod-sql01" -Path "C:\Backups\AgentAlerts"

.EXAMPLE
    Export-MSAgentAlert -SqlInstance "prod-sql01" -FileName "C:\Backups\AgentAlerts\alerts.sql"

.EXAMPLE
    Export-MSAgentAlert -SqlInstance "prod-sql01" -AgentAlert "Severity*", "Error*"
...
#>
    ...
    begin {
        ...
        $hasWriteLog = Get-Command -Name Write-MSLog -ErrorAction SilentlyContinue
        function _logInfo { param($m) if ($hasWriteLog) { Write-MSLog -Message $m -Level Info } else { Write-Verbose $m } }
        function _logWarn { param($m) if ($hasWriteLog) { Write-MSLog -Message $m -Level Warning } else { Write-Warning $m } }
        function _logErr  { param($m) if ($hasWriteLog) { Write-MSLog -Message $m -Level Error } else { Write-Error $m } }

        _logInfo "Starting Export-MSAgentAlert for instance [$SqlInstance]"
    ...
    process {
        try {
            ...
            _logInfo "Finished Export-MSAgentAlert for instance [$SqlInstance]"
        }
    }
}