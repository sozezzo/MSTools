function Invoke-MSCloneUser {
    ...
    .DESCRIPTION
        ...
        - Uses Write-MSLog (expects -Message/-Level).
    ...
    Write-MSLog -Message "Cloning users from [$SourceInstance].[$SourceDatabase] to [$DestInstance].[$DestDatabase] (Force=$Force)" -Level Info
    ...
    foreach ($u in $srcUsers) {
        ...
        if ($existsOnDest -and -not $Force) {
            Write-MSLog -Message "Skip (already exists on destination): $name" -Level Info
            ...
            if ($PSCmdlet.ShouldProcess("[$DestInstance].[$DestDatabase]", "Drop user '$name' before recreate")) {
                ...
                Write-MSLog -Message "Dropped existing user: $name (Force)" -Level Warning
            }
        }
        ...
        if ($u.Login) {
            ...
            if ($Force) {
                if ($PSCmdlet.ShouldProcess($DestInstance, "Copy/refresh login '$loginName' from $SourceInstance (Force)")) {
                    ...
                    Write-MSLog -Message "Ensured/updated login on destination: $loginName" -Level Warning
                }
                ...
                if ($PSCmdlet.ShouldProcess($DestInstance, "Copy missing login '$loginName' from $SourceInstance")) {
                    ...
                    $dstLoginNames[$loginKey] = $true
                    Write-MSLog -Message "Added missing login on destination: $loginName" -Level Warning
                }
            }
        }
        ...
        if ($null -ne $userNow) {
            if (-not $Force) {
                Write-MSLog -Message "Skip (detected user now exists after login copy): $name" -Level Info
                ...
                if ($PSCmdlet.ShouldProcess("[$DestInstance].[$DestDatabase]", "Drop user '$name' (post-check) before recreate")) {
                    Remove-DbaDbUser -SqlInstance $DestInstance -Database $DestDatabase -User $name -Confirm:$false -EnableException
                    Write-MSLog -Message "Dropped existing user (post-check): $name (Force)" -Level Warning
                }
            }
        }
        ...
        if ($PSCmdlet.ShouldProcess("[$DestInstance].[$DestDatabase]", "Create user '$($u.Name)' (Login='$($u.Login)'; DefaultSchema='$($u.DefaultSchema)'; Force=$Force)")) {
            ...
            Write-MSLog -Message ("{0} user: {1} (Login='{2}'; DefaultSchema='{3}')" -f $action, $name, $u.Login, $u.DefaultSchema) -Level Warning
            $created++
    ...
    Write-MSLog -Message ("Summary: Created={0}  Skipped={1}  Failed={2}" -f $created,$skipped,$failed) -Level Warning
    if ($PassThru) {
        [pscustomobject]@{
