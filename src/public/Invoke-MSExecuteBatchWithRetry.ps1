function Invoke-MSExecuteBatchWithRetry {
<#
.SYNOPSIS
...
.DESCRIPTION
    Invoke-MSExecuteBatchWithRetry reads a .sql file, splits it into batches on standalone
    ...
    failures, and the failed SQL are optionally logged via Write-MSLog.
...
.PARAMETER LogFile
    Optional log file path. When supplied, Write-MSLog messages and verbose streams
...
.EXAMPLES
    ...
    Invoke-MSExecuteBatchWithRetry `
    ...
    if (-not (Invoke-MSExecuteBatchWithRetry -SqlInstance "SQL02" -Database "Ops_QA" `
...
.NOTES
    ...
    Logging:
      - Uses Write-MSLog for messages. In this function, calls include positional -Message usage
        and the parameter name -LogFilePath/-LogFile interchangeably; ensure your Write-MSLog
    ...
    Dependencies:
      ...
      - Write-MSLog utility capable of arguments like:
            Write-MSLog -Message <string> [-Level <Enum/String>] [] [-LogFilePath <path>]
...
#>
    ...
    # Split on standalone GO lines (case-insensitive) (unique)
...
"@
    ...
    while ($pending.Count -gt 0 -and $pass -le $MaxPasses) {
        ...
        $pass++
        Write-MSLog "Executing $($pending.Count) batch(es)..."  
    
        $failed = @()
        foreach ($b in $pending) {
            try {
                Invoke-DbaQuery -SqlInstance $SqlInstance -Database $Database -Query $b.Sql *>> $LogFile
                Write-MSLog "$go$($b.Sql)$go"   
            }
            catch {
                $msg = $_.Exception.Message
                Write-MSLog -message "FAILED [Pass $pass, Batch $($b.Index)]: $msg"  
                $failed += $b
            }
        }
        ...
        if ($failed.Count -eq $pending.Count) {
            Write-MSLog "No progress in pass $pass; stopping. Remaining: $($failed.Count) batch(es)."   
            break
        }
        
        if ($failed.Count -eq 0)
        {
             Write-MSLog -message "All script was executed" -Level Warning  
        }
    ...
    if ($pending.Count -gt 0) {
        ...
        Write-MSLog "Some batches failed after $($pass) passes: $($pending.Count). See log for details." -Level Error

        foreach ($b in $failed) {
            Write-MSLog -message "/* FAILED SQL Script */ $go$($b.Sql)$go$($b.Sql)" 
        }
        ...
        Write-MSLog "Programmables deployed after $($pass) pass(es)."  
        return $true
