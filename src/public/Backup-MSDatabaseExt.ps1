...
.DESCRIPTION
    Backup-MSDatabaseExt allows you to back up only a subset of databases 
...
#>
    ...
    begin {
        ...
        if (-not $PSBoundParameters.ContainsKey('CompressBackup') -and -not $NoCompress) {
            $CompressBackup = $true
            Write-MSLog -Level Debug -Message "Backup-MSDatabaseExt: CompressBackup default - TRUE."
        }

        if (-not $PSBoundParameters.ContainsKey('Checksum') -and -not $NoChecksum) {
            $Checksum = $true
            Write-MSLog -Level Debug -Message "Backup-MSDatabaseExt: Checksum default - TRUE."
        }

        # Collect per-database results
        $BackupResults = @()

        Write-MSLog -Level Info -Message "Backup-MSDatabaseExt: START. Instances=[$($SqlInstance -join ', ')], RootPath=[$Path], Type=[$Type], NoTimestamp=[$NoTimestamp], CleanupOlderThanHours=[$CleanupOlderThanHours]"
    }
    ...
    process {
        ...
        foreach ($instanceName in $SqlInstance) {
            ...
            Write-MSLog -Level Info -Message "Processing instance [$instanceName] - EffectivePath=[$effectivePath]"

            # Ensure the effective path exists
            try {
                if (-not (Test-Path -LiteralPath $effectivePath)) {
                    Write-MSLog -Level Debug -Message "Creating directory [$effectivePath]."
                    New-Item -ItemType Directory -Path $effectivePath -Force | Out-Null
                }
            }
            catch {
                Write-MSLog -Level Error -Message "[$instanceName] - Failed to create/access directory [$effectivePath]. Error: $($_.Exception.Message)"
                continue
            }
            ...
            try {
                ...
                Write-MSLog -Level Debug -Message "[$instanceName] - [$($allDbs.Count)] user DB(s) found."
            }
            catch {
                Write-MSLog -Level Error -Message "[$instanceName] - Get-DbaDatabase FAILED. Error: $($_.Exception.Message)"
                continue
            }

            if (-not $allDbs) {
                Write-MSLog -Level Warning -Message "[$instanceName] - No user databases found."
                continue
            }
            ...
            if ($Type -eq 'Log') {
                ...
                if ($after -lt $before) {
                    Write-MSLog -Level Info -Message "[$instanceName] - Excluded [$($before - $after)] SIMPLE recovery DB(s) from log backup."
                }

                if (-not $allDbs) {
                    Write-MSLog -Level Warning -Message "[$instanceName] - No eligible databases (non-SIMPLE) remain for log backup."
                    continue
                }
            }
            ...
            if ($Database.Count -eq 1 -and $Database[0] -eq '*') {
                $dbs = $allDbs
                Write-MSLog -Level Info -Message "[$instanceName] - Using ALL eligible user DBs (no name filter)."
            }
            else {
                ...
                if (-not $dbs) {
                    Write-MSLog -Level Warning -Message "[$instanceName] - No databases match filter(s): $($Database -join ', ')"
                    continue
                }

                Write-MSLog -Level Info -Message "[$instanceName] - Matching DBs: $($dbs.Name -join ', ')"
            }
            ...
            foreach ($db in $dbs) {
                ...
                if ($isOffline) {
                    Write-MSLog -Level Warning -Message "[$instanceName] - Database [$dbName] is OFFLINE. Backup will be skipped and reported as Offline."
                ...
                if (-not $PSCmdlet.ShouldProcess($instanceName, $targetText)) {
                    Write-MSLog -Level Info -Message "[$instanceName] - ShouldProcess declined for DB [$dbName] (WhatIf?)."
                    continue
                }
                ...
                if ($NoTimestamp) {
                    ...
                    Write-MSLog -Level Info -Message "[$instanceName] - NoTimestamp=TRUE, DB=[$dbName], FilePath=[$filePath]"
                }
                else {
                ...
                Write-MSLog -Level Debug -Message "[$instanceName] - Backup-DbaDatabase params for DB [$dbName]: $paramText"
                ...
                try {
                    ...
                    Write-MSLog -Level Info -Message "[$instanceName] - Backup completed for DB [$dbName]."
                }
                catch {
                    $errorMessage = $_.Exception.Message
                    Write-MSLog -Level Error -Message "[$instanceName] - Backup FAILED for DB [$dbName]. Error: $errorMessage"
                    ...
                    Write-MSLog -Level Debug -Message "[$instanceName] - Compression error check for DB [$dbName]."
                    Write-MSLog -Level Debug -Message "--------"
                    Write-MSLog -Level Debug -Message $errorMessage
                    Write-MSLog -Level Debug -Message "--------"
                    ...
                    if ($isCompressionError -and -not $OutputScriptOnly) {
                        Write-MSLog -Level Warning -Message "[$instanceName] - Compression/media mismatch for DB [$dbName]. Retrying backup WITHOUT compression."
                        ...
                        try {
                            ...
                            $errorMessage          = $null
                            Write-MSLog -Level Info -Message "[$instanceName] - Backup completed for DB [$dbName] on retry WITHOUT compression."
                        }
                        catch {
                            $errorMessage = $_.Exception.Message
                            Write-MSLog -Level Error -Message "[$instanceName] - Retry WITHOUT compression FAILED for DB [$dbName]. Error: $errorMessage"
                        }
                    }
                }
                ...
                if ($backupSucceeded -and $CleanupOlderThanHours -gt 0 -and -not $OutputScriptOnly) {
                    try {
                        ...
                        Write-MSLog -Level Info -Message "[$instanceName] - Cleanup for DB [$dbName]: deleting *.bak/*.trn older than [$CleanupOlderThanHours] hour(s) under [$effectivePath]."
                        ...
                        if ($files -and $files.Count -gt 0) {
                            Write-MSLog -Level Info -Message "[$instanceName] - Cleanup for DB [$dbName]: found [$($files.Count)] file(s) to delete."

                            foreach ($file in $files) {
                                Write-MSLog -Level Debug -Message "[$instanceName] - Cleanup for DB [$dbName]: deleting [$($file.FullName)] (LastWriteTime=$($file.LastWriteTime))"
                                Remove-Item -LiteralPath $file.FullName -Force
                            }

                            Write-MSLog -Level Info -Message "[$instanceName] - Cleanup for DB [$dbName] completed under [$effectivePath]."
                        }
                        else {
                            Write-MSLog -Level Info -Message "[$instanceName] - Cleanup for DB [$dbName]: no files older than [$CleanupOlderThanHours] hour(s) under [$effectivePath]."
                        }
                    }
                    catch {
                        Write-MSLog -Level Error -Message "[$instanceName] - Cleanup for DB [$dbName] FAILED under [$effectivePath]. Error: $($_.Exception.Message)"
                    }
                }
                elseif ($CleanupOlderThanHours -gt 0 -and $OutputScriptOnly) {
                    Write-MSLog -Level Debug -Message "[$instanceName] - Cleanup for DB [$dbName] skipped (OutputScriptOnly is set)."
                }
            }
        }
    }
        ...
        Write-MSLog -Level Info -Message "Backup-MSDatabaseExt: Summary - Total=[$total], Success=[$success], Failed=[$failed]."
