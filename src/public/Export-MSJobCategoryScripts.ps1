function Export-MSJobCategoryScripts {
<#
.SYNOPSIS
...
.EXAMPLE
    Export-MSJobCategoryScripts -SqlInstance "SQL01" -Path "C:\out"

.EXAMPLE
    Export-MSJobCategoryScripts -SqlInstance "SQL01" -FileName "C:\out\category.ALL.sql"

.EXAMPLE
    Export-MSJobCategoryScripts -SqlInstance "SQL01" -Path "C:\out" -Category "Database Maintenance","Backups"
    ...
    try {
        ...
        if ([string]::IsNullOrWhiteSpace($FileName)) {
            if ([string]::IsNullOrWhiteSpace($Path)) {
                ...
                Write-MSLog -Message $msg -Level Error
                throw $msg
            }
        ...
        Write-MSLog -Message "Reading categories from [$SqlInstance]..." -Level Info
...
"@
        ...
        if (-not $rows -or $rows.Count -eq 0) {
            Write-MSLog -Message "No categories matched filters. Nothing to export." -Level Warning
            return
        }
...
"@
            ...
            Write-MSLog -Message "Prepared category [$name] (Class=$class)" -Level Debug
        }
        ...
        if ($PSCmdlet.ShouldProcess($FileName, "Write category script")) {
            ...
            Write-MSLog -Message "Saved category script to: $FileName" -Level Info
        }
    }
    catch {
        Write-MSLog -Message "Error in Export-MSJobCategoryScripts: $($_.Exception.Message)" -Level Error
    }
}
