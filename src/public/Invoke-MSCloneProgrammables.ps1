function Invoke-MSCloneProgrammables {
<#
.SYNOPSIS
...
.DESCRIPTION
    Invoke-MSCloneProgrammables generates two script files:
    ...
    Invoke-MSExecuteBatchWithRetry to handle inter-batch dependencies and transient failures.
    ...
    Key behaviors:
      ...
      - Writes progress and results via Write-MSLog.
...
.PARAMETER MaxPasses
    Maximum number of retry passes used by Invoke-MSExecuteBatchWithRetry during deployment.
    Default: 10.

.PARAMETER LogFileName
    Optional log file path. Verbose streams and Write-MSLog messages are appended here.
...
#>	
    ...
    try {
        ...
        Write-MSLog -Message "Copy programmable objects (views, functions, procedures, synonyms, DDL triggers) into new database" -Level Info
        ...
        # UDFs
        Write-MSLog -Message "Exporting Functions" -Level Info 
        ...
        # Views
        Write-MSLog -Message "Exporting Views" -Level Info
        ...
        # TABLE triggers
        Write-MSLog -Message "Exporting DML triggers (table triggers)" -Level Info 
        ...
        # VIEW triggers
        Write-MSLog -Message "Exporting DML triggers (VIEW triggers)" -Level Info 
        Get-DbaDbView -SqlInstance $SourceInstance -Database $SourceDatabase -ExcludeSystemView |
            Export-DbaScript -FilePath $ScriptPath -Append:$true  -ScriptingOptionsObject $optionsTrig -EnableException -Confirm:$false *>> $LogFileName
        ...
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy programmables")) {
            Write-MSLog -Message "Deploying programmables into $DestDatabase on $DestInstance" -Level Warning 
        ...
        Write-MSLog -Message "Programmables deployment completed." -Level Warning 

    }
    catch {
        Write-MSLog -Message ("ERROR (Invoke-MSCloneProgrammables): " + $_.Exception.Message) -Level Error 
        throw
    }
}
