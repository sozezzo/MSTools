...
.NOTES
    ...
    Dependencies:
      ...
      - Write-Log utility capable of arguments like:
            Write-Log -Message <string> [-Level <Enum/String>] [] [-LogFilePath <path>]
...
"@
    ...
    while ($pending.Count -gt 0 -and $pass -le $MaxPasses) {
        ...
        $pass++
        Write-Log "Executing $($pending.Count) batch(es)..."  
    
        $failed = @()
        foreach ($b in $pending) {
            try {
                Invoke-DbaQuery -SqlInstance $SqlInstance -Database $Database -Query $b.Sql *>> $LogFile
                Write-Log "$go$($b.Sql)$go"   
            }
            catch {
                $msg = $_.Exception.Message
                Write-Log -message "FAILED [Pass $pass, Batch $($b.Index)]: $msg"  
                $failed += $b
            }
        }
        ...
        if ($failed.Count -eq $pending.Count) {
            Write-Log "No progress in pass $pass; stopping. Remaining: $($failed.Count) batch(es)."   
            break
        }
        
        if ($failed.Count -eq 0)
        {
             Write-Log -message "All script was executed" -Level Warning  
        }
    ...
    if ($pending.Count -gt 0) {
        ...
        Write-Log "Some batches failed after $($pass) passes: $($pending.Count). See log for details." -Level Error

        foreach ($b in $failed) {
            Write-Log -message "/* FAILED SQL Script */ $go$($b.Sql)$go$($b.Sql)" 
        }
        ...
        Write-Log "Programmables deployed after $($pass) pass(es)."  
        return $true
