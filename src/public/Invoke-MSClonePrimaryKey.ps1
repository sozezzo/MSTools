function Invoke-MSClonePrimaryKey {
<#
.SYNOPSIS
    Scripts and deploys ONLY Primary Keys (PK) from a source database to a destination database.

.DESCRIPTION
    Invoke-MSClonePrimaryKey enumerates tables in the source database (optionally filtered),
...
.PARAMETER MaxPasses
    Maximum number of retry passes for the deployment executor (Invoke-MSExecuteBatchWithRetry).
    Default: 10.
...
.NOTES
    ...
    Error handling:
      - (Optional) try/catch can wrap the entire function; inner code logs via Write-MSLog.
      - Deployment uses Invoke-MSExecuteBatchWithRetry for robustness.
...
#>
    ...
    #try {
        Write-MSLog -Message "Exporting ONLY Primary Keys (no table creation)" -Level Info  
        ...
        Write-MSLog -Message "Found $($pkIndexes.Count) primary keys to script." -Level Info  
        ...
        Write-MSLog -Message "Primary Key script written: $ScriptPath" -Level Info  

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy Primary Keys")) {
            Write-MSLog -Message "Deploying Primary Keys into $DestDatabase on $DestInstance" -Level Warning  
        ...
        Write-MSLog -Message "Primary Key deployment completed." -Level Warning  
    ...
    #}
    #catch {
    #    Write-MSLog -Message ("ERROR (Invoke-MSClonePrimaryKey): " + $_.Exception.Message) -Level Error -LogFileName $LogFile
    #    throw
    #}
}
