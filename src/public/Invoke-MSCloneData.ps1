...
.DESCRIPTION
    Invoke-MSCloneData discovers user tables on the source (optionally filtered by schema/table),
    ...
    auto-create missing destination tables. Progress and errors are written via Write-MSLog. Failures
...
.PARAMETER LogFileName
    Optional path to a log file. Verbose streams and Write-MSLog messages will be appended here.
...
.NOTES
    Requirements:
      - dbatools module:
            Get-DbaDbTable
            Copy-DbaDbTableData
      - A Write-MSLog function that supports: -Message, -Level, -LogFileName.
      - Permissions:
    ...
    Logging:
      - Writes start/stop timestamps, per-table progress, duration, and OK/FAIL counts via Write-MSLog.
...
#>
    ...
    try {
        Write-MSLog -Message "Copy all user tables from $SourceDatabase [$SourceInstance] to $DestDatabase [$DestInstance]" -Level Info 
        if ($LogFileName) { Write-MSLog -Message "Log file: $LogFileName" -Level Info -LogFileName $LogFileName }
        ...
        Write-MSLog -Message "Tables selected for copy: $total" -Level Info 
        ...
        foreach ($t in $tables) {
            $i++
            $fq = "[$($t.Schema)].[$($t.Name)]"
            Write-MSLog -Message "[$i/$total] Copying $fq ..." -Level Info 
            ...
            if ($PSCmdlet.ShouldProcess("$DestInstance/$DestDatabase", "Copy $fq")) {
                try {
                    ...
                    Write-MSLog -Message "FAILED to copy $fq : $msg" -Level Error 
        ...
        Write-MSLog -Message ("Duration to copy all user tables: {0:c}" -f $dur) -Level Warning 
        Write-MSLog -Message ("Summary: OK={0}  FAIL={1}" -f $ok.Count,$failed.Count) -Level Warning 

        if ($failed.Count -gt 0) {
            $failed | ForEach-Object { Write-MSLog -Message ("  - " + $_.Table + " :: " + $_.Error) -Level Warning }
        }
        ...
        Write-MSLog -Message ("ERROR (Invoke-MSCloneData): " + $_.Exception.Message) -Level Error
        throw
    }
}