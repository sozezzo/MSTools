function Set-MSDbServiceBroker {
<#
.SYNOPSIS
    Enables, disables, or resets (NEW_BROKER) the Service Broker state for one or more SQL Server databases.

.DESCRIPTION
    Wraps ALTER DATABASE ... SET { ENABLE_BROKER | DISABLE_BROKER | NEW_BROKER } using dbatools helpers.
    Supports multiple databases (names, wildcards, or -AllUser), exclusions, WITH ROLLBACK IMMEDIATE,
    WhatIf/Confirm, and returns pre/post state.

.PARAMETER SqlInstance
    Target SQL Server instance (string like "Server\Instance" or "Server").

.PARAMETER SqlCredential
    PSCredential to connect (SQL or Windows). Passed through to dbatools commands.

.PARAMETER Database
    One or more database names. Wildcards supported. You can also pipe plain strings.
...
#>
    ...
    begin {
        ...
        if (-not (Get-Module -ListAvailable -Name dbatools)) {
            throw "dbatools module not found. Install-Module dbatools; then Import-Module dbatools."
        }
        ...
        function Resolve-DbList {
            ...
            # Baseline set (all user DBs or union of wildcards)
...
"@
    ...
    process {
        ...
        foreach ($t in $targets) {
            ...
            # Build ALTER DATABASE and ShouldProcess message
