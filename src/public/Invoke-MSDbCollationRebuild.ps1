function Invoke-MSDbCollationRebuild {
    <#
    .SYNOPSIS
    ...
    begin {
        ...
        Write-MSLog -Message "Start database collation rebuild: [$SourceInstance].[$SourceDatabase] -> [$DestInstance].[$NewDatabase] ($TargetCollation)" -Level Info -LogFileName $LogFileName -Verbose
    }
    ...
    process {
        ...
        if ($DoInitialCopy) {
            ...
            Write-MSLog -Message "Initial copy via $InitialCopyMethod to [$DestInstance].[$newName]" -Level Info -LogFileName $LogFileName -Verbose
        ...
        if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "Set compatibility level $CompatibilityLevel")) {
            Write-MSLog -Message "Setting DB compatibility to $CompatibilityLevel." -Level Info -LogFileName $LogFileName
        ...
        if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "Enable ALLOW_SNAPSHOT_ISOLATION")) {
            Set-MSDbSnapshotIsolation -SqlInstance $DestInstance -Database $NewDatabase -Enable
        }

        # Enable Service Broker (optional but you had it)
        if ($PSCmdlet.ShouldProcess("$DestInstance / $NewDatabase", "ENABLE_BROKER")) {
            Set-MSDbServiceBroker -SqlInstance $DestInstance -Database $NewDatabase -EnableBroker
        }
        ...
        Invoke-MSClonePrimaryKey -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
            -DestInstance $DestInstance -DestDatabase $NewDatabase -PkScriptPath $pkScript -MaxPasses 10 -LogFileName $LogFileName | Out-Null

        Invoke-MSCloneIndexes -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
        ...
        Invoke-MSClonePkFk -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
        ...
        Invoke-MSCloneConstraint -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
        ...
        Invoke-MSCloneProgrammables -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
            -DestInstance $DestInstance -DestDatabase $NewDatabase `
            -ProgrammablesPath $progScript -DmlTriggersPath $trigDmlScript -MaxPasses 10 -LogFileName $LogFileName | Out-Null

        # 7) Users
        Invoke-MSCloneUser -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
            -DestInstance $DestInstance -DestDatabase $NewDatabase -LogFileName $LogFileName | Out-Null

        # 8) Diff
        $diff = Compare-MSDbObject -SourceInstance $SourceInstance -SourceDatabase $SourceDatabase `
                                  -DestInstance $DestInstance   -DestDatabase   $NewDatabase

        $end = Get-Date
        $dur = $end - $beginScript

        Write-MSLog -Message "End collation rebuild. Duration: $dur" -Level Info -LogFileName $LogFileName -Verbose
