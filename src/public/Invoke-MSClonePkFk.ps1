...
.DESCRIPTION
    Invoke-MSClonePkFk uses dbatools/SMO to:
...
.PARAMETER LogFileName
    ...
    and errors to this file via Write-MSLog and redirected verbose streams.
...
.NOTES
    Requirements:
      ...
      - A Write-MSLog function that accepts -Message, -Level, -LogFileName (as used here).
      - A helper Invoke-MSExecuteBatchWithRetry that takes:
...
#>
    ...
    try {
        Write-MSLog -Message "Exporting Indexes + PK + FK (no table creation)" -Level Info 
        ...
        Write-MSLog -Message "Index/PK/FK script: $IdxPkFkScriptPath (Indexes=$($indexObjects.Count), FKs=$($fkObjects.Count))" -Level Info 

        # Deploy with retry
        if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy Indexes + PK + FK")) {
            Write-MSLog -Message "Deploying Indexes + PK + FK into $DestDatabase on $DestInstance" -Level Warning 
            [void](Invoke-MSExecuteBatchWithRetry -SqlInstance $DestInstance -Database $DestDatabase -FilePath $IdxPkFkScriptPath -MaxPasses $MaxPasses)
        }

        Write-MSLog -Message "Indexes + PK + FK deployment completed." -Level Warning 
        ...
        Write-MSLog -Message ("ERROR (Invoke-MSClonePkFk): " + $_.Exception.Message) -Level Error 
        throw
    }
}
