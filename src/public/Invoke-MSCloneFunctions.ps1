function Invoke-MSCloneFunctions {
<#
.SYNOPSIS
...
.PARAMETER MaxPasses
    Maximum retry passes for Invoke-MSExecuteBatchWithRetry. Default: 10.

.PARAMETER LogFileName
    Optional log file path; if omitted, uses $Global:WriteLog_LogFileName when present.

.EXAMPLE
    Invoke-MSCloneFunctions -SourceInstance SQL01 -SourceDatabase AppDB `
                             -DestInstance SQL02 -DestDatabase AppDB_Stage `
                             -ScriptPath 'C:\out\AppDB-functions.sql'

.EXAMPLE
    Invoke-MSCloneFunctions -SourceInstance SQL01 -SourceDatabase Sales `
...
#>
    ...
    begin {
        ...
        try {
            ...
            Write-MSLog -Message "WARN: Could not determine destination version: $($_.Exception.Message). Assuming no DROP IF EXISTS." -Level Warning
        }

        if ($UseDropIfExists -and -not $dropIfExistsSupported) {
            Write-MSLog -Message "Destination does not support DROP IF EXISTS (requires SQL 2016+). Falling back to IncludeIfNotExists." -Level Warning
    ...
    process {
        try {
            Write-MSLog -Message "Collecting UDFs from [$SourceInstance].[$SourceDatabase]." -Level Info
            ...
            if (-not $targetUdfs -or $targetUdfs.Count -eq 0) {
                Write-MSLog -Message "No UDFs matched the provided filter." -Level Warning
                return
            }

            Write-MSLog -Message ("UDFs selected: " + ($targetUdfs | ForEach-Object { "$($_.Schema).$($_.Name)" } | Sort-Object | Out-String).Trim()) -Level Info
            ...
            if ($UseDropIfExists) {
                Write-MSLog -Message "Scripting with DROP FUNCTION IF EXISTS prelude per object." -Level Info
                ...
                Write-MSLog -Message "Scripting using IncludeIfNotExists wrapper (SMO)." -Level Info
            ...
            if ($PSCmdlet.ShouldProcess("$DestInstance / $DestDatabase", "Deploy functions")) {
                Write-MSLog -Message "Deploying functions into [$DestInstance].[$DestDatabase]." -Level Warning
                [void](Invoke-MSExecuteBatchWithRetry -SqlInstance $DestInstance -Database $DestDatabase -FilePath $ScriptPath -MaxPasses $MaxPasses)
                Write-MSLog -Message "Functions deployment completed." -Level Warning
            } else {
                Write-MSLog -Message "Scripting completed (no deployment due to -WhatIf / -Confirm choice)." -Level Info
            }
        }
        catch {
            Write-MSLog -Message ("ERROR (Invoke-MSCloneFunctions): " + $_.Exception.Message) -Level Error
            throw
        }
    }
}
